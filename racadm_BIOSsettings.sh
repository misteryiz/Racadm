...
racadm_pending () {
	...
	if [ "$check_pen" = Pending ] && [ -z "$job_pen" ];then
	    jobqueue ${ip_list[i]} "$jobqueuetarget"
            ...
            echo -e "\n\tChanging $target to $value : \n"
            sshpass -p$pass ssh -o StrictHostKeyChecking=no $login@${ip_list[i]} racadm set $target $value &> /tmp/error_func.$$
	    error_func ${ip_list[i]} $target $jobqueuetarget
	fi
    fi
(( pend_count += 1 ))
}
...
if [ -f "/tmp/error_func.$$" ];then
grep -i error /tmp/error_func.$$
if [ $? = 0 ];then
echo -e "\n${R}$1 : !!Error!! for $2..You may have re-try this node ..${N}"
echo $1 >> $rerun_log_file
else
jobqueue $1 $3
fi
fi
